...
class EnergyEnv:
    ...
    def get_state(self):
        h = self.hour
        return np.array([self.T/30,outdoor_temperature(h)/30, self.non_shiftable[0] ,self.EV_SOC,self.SOC, pv_profile(h), price_profile(h)/0.4,np.sin(2*np.pi*h/24),np.cos(2*np.pi*h/24)], dtype=np.float32)
